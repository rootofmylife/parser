...
class TransferParser(object):
    ...
    def __init__(self, args, model, transform, transfer):
        self.args = args
        self.model = model
    ...
    def train(self, train, dev, test, buckets=32, batch_size=5000, update_steps=1,
        ...
        self.transfer.train() # TRANSFER LEARNING
        batch_size = batch_size // update_steps
        if dist.is_initialized():
            batch_size = batch_size // dist.get_world_size()

        # TRANSFER LEARNING
        logger.info("Loading the transfer data")
        add = Dataset(self.transfer, args.transfer, **args).build(batch_size, buckets, True, dist.is_initialized())
        ...
        if args.encoder == 'lstm':
            ...
            steps = (len(train.loader) + len(add.loader)) * epochs // args.update_steps
        ...
        for epoch in range(self.epoch, args.epochs + 1):
            ...
            self._train(add.loader) # train on addtional data
